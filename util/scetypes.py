...
class SceControlInfoDigest256:
    ...
    def __str__(self):
        ...
        ret += ' SCE Hash:         {0}\n'.format(self.sce_hash.hex())
        ret += ' File Hash:        {0}\n'.format(self.file_hash.hex())
...
class SceControlInfoDRM:
    ...
    def __str__(self):
        ...
        ret += ' Type Digest:       {0}\n'.format(self.digest1.hex())
        ret += ' ECDSA224 Sig R:    {0}\n'.format(self.sig2r.hex())
        ret += ' ECDSA224 Sig S:    {0}\n'.format(self.sig2s.hex())
        return ret                   
...
class SceRIF:
    ...
    def __str__(self):
        ...
        ret += ' CID:               0x{0:X}\n'.format(self.cid)
        ret += ' Content ID:        {0}\n'.format(self.content_id)
        ret += ' KLicensee:         {0}\n'.format(self.klicense.hex())
        ret += ' ECDSA160 Sig R:    {0}\n'.format(self.sig1r.hex())
        ret += ' ECDSA160 Sig S:    {0}\n'.format(self.sig1s.hex())
        return ret                           